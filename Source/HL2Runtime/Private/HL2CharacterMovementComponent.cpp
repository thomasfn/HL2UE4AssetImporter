...
UHL2CharacterMovementComponent::UHL2CharacterMovementComponent()
{
...
void UHL2CharacterMovementComponent::SetSprinting(bool NewSprinting)
{
	bSprinting = NewSprinting;
}

void UHL2CharacterMovementComponent::SetWalking(bool NewWalking)
{
	bWalking = NewWalking;
}

void UHL2CharacterMovementComponent::TickComponent(float DeltaTime, enum ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
...
bool UHL2CharacterMovementComponent::DoJump(bool bClientSimulation)
{
	return bCheatFlying || Super::DoJump(bClientSimulation);
}

#if MID_AIR_STEP
void UHL2CharacterMovementComponent::PhysFalling(float deltaTime, int32 Iterations)
{
...
bool UHL2CharacterMovementComponent::CanStepUp(const FHitResult& Hit) const
{
...
bool UHL2CharacterMovementComponent::StepUp(const FVector& GravDir, const FVector& Delta, const FHitResult& InHit, FStepDownResult* OutStepDownResult)
{
...
void UHL2CharacterMovementComponent::TwoWallAdjust(FVector& OutDelta, const FHitResult& Hit, const FVector& OldHitNormal) const
{
	Super::Super::TwoWallAdjust(OutDelta, Hit, OldHitNormal);
}

float UHL2CharacterMovementComponent::SlideAlongSurface(const FVector& Delta, float Time, const FVector& Normal, FHitResult& Hit, bool bHandleImpact)
{
	return Super::Super::SlideAlongSurface(Delta, Time, Normal, Hit, bHandleImpact);
}

FVector UHL2CharacterMovementComponent::HandleSlopeBoosting(const FVector& SlideResult, const FVector& Delta, const float Time, const FVector& Normal, const FHitResult& Hit) const
{
	return SlideResult;
}

bool UHL2CharacterMovementComponent::ShouldCatchAir(const FFindFloorResult& OldFloor, const FFindFloorResult& NewFloor)
{
...
void UHL2CharacterMovementComponent::OnMovementModeChanged(EMovementMode PreviousMovementMode, uint8 PreviousCustomMode)
{
	...
	FHL2Footstep footstep;
	footstep.PhysicalMaterial = Hit.PhysMaterial.Get();
	footstep.Type = bJumped ? EHL2FootstepType::Jump : EHL2FootstepType::Landing;
	footstep.Strength = MoveSoundVolume;
	OnFootstep.Broadcast(footstep);
}

void UHL2CharacterMovementComponent::ToggleNoClip()
{
	if (bCheatFlying)
	{
...
void UHL2CharacterMovementComponent::ApplyVelocityBraking(float DeltaTime, float Friction, float BrakingDeceleration)
{
	float Speed = Velocity.Size2D();
...
void UHL2CharacterMovementComponent::PlayMoveSound(float DeltaTime)
{
	...
	bool bIsSprinting = Speed >= SprintSpeedThreshold * SprintSpeedThreshold;
	float MoveSoundVolume = 1.0f;
	...
	{
		MoveSoundTime = bIsSprinting ? 300.0f : 400.0f;
	...
	FHL2Footstep footstep;
	footstep.PhysicalMaterial = CurrentFloor.HitResult.PhysMaterial.Get();
	footstep.Type = StepSide ? EHL2FootstepType::RightStep : EHL2FootstepType::LeftStep;
	footstep.Strength = MoveSoundVolume;
	OnFootstep.Broadcast(footstep);
...
#if WIP_SURFING
void UHL2CharacterMovementComponent::PreemptCollision(float DeltaTime, float SurfaceFriction)
{
...
void UHL2CharacterMovementComponent::CalcVelocity(float DeltaTime, float Friction, bool bFluid, float BrakingDeceleration)
{
	...
	if (bCheatFlying)
	{
		...
		else
		{
			...
			auto NoClipAccelClamp = IsSprinting() ? 2.0f * MaxAcceleration : MaxAcceleration;
	...
	else
	{
		...
		if (SpeedSq <= MaxWalkSpeedCrouched * MaxWalkSpeedCrouched)
		{
			...
			MaxStepHeight = GetClass()->GetDefaultObject<UHL2CharacterMovementComponent>()->MaxStepHeight;
			SetWalkableFloorZ(GetClass()->GetDefaultObject<UHL2CharacterMovementComponent>()->GetWalkableFloorZ());
		}
		else
		{
			...
			MaxStepHeight = FMath::Clamp(GetClass()->GetDefaultObject<UHL2CharacterMovementComponent>()->MaxStepHeight * (1.0f - SpeedMultiplier), MinStepHeight,
										 GetClass()->GetDefaultObject<UHL2CharacterMovementComponent>()->MaxStepHeight);
			SetWalkableFloorZ(FMath::Clamp(GetClass()->GetDefaultObject<UHL2CharacterMovementComponent>()->GetWalkableFloorZ() - (0.5f * (0.4f - SpeedMultiplier)),
										   GetClass()->GetDefaultObject<UHL2CharacterMovementComponent>()->GetWalkableFloorZ(), 0.9848f));
		}
	}
...
void UHL2CharacterMovementComponent::Crouch(bool bClientSimulation)
{
	bInCrouch = true;
}

void UHL2CharacterMovementComponent::DoCrouchResize(float TargetTime, float DeltaTime, bool bClientSimulation)
{
...
void UHL2CharacterMovementComponent::UnCrouch(bool bClientSimulation)
{
	bInCrouch = true;
}

void UHL2CharacterMovementComponent::DoUnCrouchResize(float TargetTime, float DeltaTime, bool bClientSimulation)
{
	if (!HasValidData())
	{
		bInCrouch = false;
...
float UHL2CharacterMovementComponent::GetMaxSpeed() const
{
